...
function customObjectFromEntries(entries) {
  ...
  const mapEnt = entries.map(([key, value]) => {
  ...
  // Використання методу `fromEntries` для створення об'єкта з масиву записів
    const obj = Object.fromEntries(mapEnt);
...
function filterObjectsByKey(arr, key) {
  ...
  const foundKey = arr.filter((obj) => obj.hasOwnProperty(key));
...
function getObjectValuesSum(obj) {
  ...
  const sumObj = obj1.reduce((accumulatar, current) => accumulatar + current ,0 );
